...
class DiffusionModelUNet(nn.Module):
    ...
    def forward(
        ...
        print("middle block shape is", h.shape)
        ...
        for upsample_block in self.up_blocks:
            ...
            print("解码器的shape是",h.shape,"hidden state的形状是",res_samples[0].shape)

        # 7. output block
        h = self.out(h)
        print("Output shape is",h.shape)
